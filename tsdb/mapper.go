...
func (pq rawPoints) Swap(i, j int) { pq[i], pq[j] = pq[j], pq[i] }
...
type tagSetCursor struct {
	...
	// Lookahead queue of points from the cursors, implemented using a heap.
	// Performance profiling shows that it is critical that this queue is part
...
func (tsc *tagSetCursor) Next(tmin, tmax int64, selectFields, whereFields []string) (int64, interface{}) {
	for {
		...
		if nextKey != -1 {
			...
			heap.Push(tsc.pqueue, nextPoint)
		}
...
func (tsc *tagSetCursor) decodeRawPoint(p *rawPoint, selectFields, whereFields []string) interface{} {
	var value interface{}
